...
def f_igso3(omega, t, L=500):
    ...
    Args:
        ...
        L: Truncation level
    """
    ls = torch.arange(L, dtype=omega.dtype, device=omega.device)[None]  # of shape [1, L]
...
def d_logf_d_omega(omega, t, L=500):
    ...
    # if t is a tensor, add a lagging dimension to it
    if torch.is_tensor(t):
        t = t.unsqueeze(-1)
    else:
        t = torch.tensor(t, dtype=omega.dtype, device=omega.device)

    # The approxmation below to the derivative at t<0.3 is very close to the true value
    return torch.where(t<0.3, -omega/t, d_f_dx / f)
...
class IGSO3:
    ...
    def sample(self, t: float, n_samples: float=1, device='cpu'):
        ...
        Returns:
            [n_samples, 3, 3] rotation matrix sampled from IGSO(3) as torch
                tensor
        """
        x = torch.randn(n_samples, 3, device=device, dtype=torch.float64)
        x /= torch.linalg.norm(x, dim=-1, keepdims=True)
        angle_sample = self.sample_angle(t, n_samples=n_samples)[:, None]
        rot_vecs = x * torch.tensor(angle_sample, device=x.device, dtype=x.dtype)
